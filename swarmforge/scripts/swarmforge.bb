...
(load-file (str (fs/path swarmforge-script-dir "agent_preflight.bb")))

(defn check-backend-dependencies! [ctx]
  (doseq [agent (map :agent (:roles ctx))]
    (check-dependency! agent))
  (agent-preflight/check! (:roles ctx)))
...
(defn run-project! [root]
  ...
  (check-dependency! "git")
  (check-dependency! "bb")
  (when (and (= "1" (System/getenv "SWARMFORGE_IN_SANDBOX"))
             (= ":99" (System/getenv "DISPLAY"))
             (not (fs/exists? "/tmp/.X11-unix/X99")))
    (check-dependency! "Xvfb")
    (process/process ["Xvfb" ":99" "-screen" "0" "1440x900x24" "-nolisten" "tcp"]
                     {:out "/tmp/swarmforge-display.log" :err :out}))
  (let [ctx (-> (context root)
                detect-tmux-base-indexes)]
    (initialize-git-repo! ctx)
    (ensure-runtime-git-excludes! ctx)
