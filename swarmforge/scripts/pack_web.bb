...
(defn sh [& args] (apply project-runtime/sh args))
(try
  (require 'safe-paths)
  (catch Exception _
...
(declare session-name pane-target live-pane-text role-row pane-sample backend-name
         ...
         session-alive? work-entry file-view integration-view)
...
(load-file (str (fs/path script-dir "pack_web_integrations.bb")))

(defn handle-post [root uri body]
  (cond
    (= "/api/integrations" uri) (post-integration root body)
...
(defn serve! [root port-str]
  (let [root (require-root! root)
    ...
    (when (forge/forge? root) (run-integrations! root))
