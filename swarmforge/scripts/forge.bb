...
(defn git-output [dir & args]
  (let [result (apply sh (concat [{:continue true} "git" "-c" "core.hooksPath=/dev/null"
                                 "-c" "core.fsmonitor=false" "-C" (str dir)] args))]
...
(defn record-managed-files! [project]
  (when-let [dir (project-runtime/host-dir project)]
    (project-runtime/write-edn! (fs/path dir "managed.edn")
      {:commit (git-output project "rev-parse" "HEAD")
       :files (vec (remove str/blank? (str/split (git-output project "ls-files" "-z" "--"
                                                         "swarmforge" "mission.md" ".gitignore") #"\u0000")))})))

(defn active-integration? [project]
  (when-let [dir (project-runtime/host-dir project)]
    (some #(not (#{:merged :rejected} (:status %)))
          (:batches (project-runtime/read-edn (fs/path dir "state.edn") {})))))
...
(defn stop-project-runtime! [forge name]
  (let [dest (str (project-dir forge name))
    ...
    (if (project-runtime/sandboxed? dest)
      (let [result (project-runtime/run dest "bb"
                    (str (fs/path dest "swarmforge/scripts/swarmforge.bb")) "--stop-project" dest)]
...
(defn local-runtime-ready? [project]
...
(defn local-runtime-stopped? [project]
  (let [socket (trimmed-file (fs/path project ".swarmforge" "tmux-socket"))]
...
(defn runtime-ready? [project]
  (if (project-runtime/sandboxed? project)
    (try (and (project-runtime/running? (project-runtime/sandbox-state project))
              (project-runtime/runtime-check project "ready")) (catch Exception _ false))
    (local-runtime-ready? project)))

(defn runtime-stopped? [project]
  (if (project-runtime/sandboxed? project)
    (let [entry (project-runtime/sandbox-state project)]
      (or (project-runtime/stopped? entry)
          (and (project-runtime/running? entry)
               (project-runtime/runtime-check project "stopped"))))
    (local-runtime-stopped? project)))
...
(defn open-project! [forge name]
  ...
  (locking project-state-lock
    (let [project (project-dir forge name)
      ...
      (try
        ;; An interrupted batch resumes its existing role worktrees and source.
        ;; Refreshing managed files here would erase evidence before publication.
        (when-not (active-integration? project)
          (refresh! forge name)
          (commit-managed-project-state! project)
          (record-managed-files! project))
        (if (skip-start?)
        ...
        (catch Exception e
          (when-not (skip-start?)
            (try (stop-project-runtime! forge name) (catch Exception _ nil)))
  ...
  (locking project-state-lock
    (doseq [[name entry] (read-project-states forge)
      ...
      (cond
        ...
        (catch Exception e
          (set-project-state! forge name "error" (.getMessage e) entry)))))
